...
test('OpenAPI 请求和响应包含完整字段类型与示例', async () => {
    ...
    assert.ok(document.paths['/dept/enums']?.get, '部门组织必须提供静态枚举接口')
    assert.ok(document.paths['/role/enums']?.get, '角色权限必须提供静态枚举接口')
    ...
    assert.ok(document.paths['/user/enums']?.get, '账号管理必须提供静态枚举接口')
