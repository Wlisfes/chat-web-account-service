...
- 变更内容：已带密码的 `REDIS_URL` 保持优先；URL 只有主机或用户名、另有 `REDIS_PASSWORD` 时由应用合并认证信息。仅当 Redis 目标能匹配同机容器名称或网络别名、Account 未显式配置密码时，部署脚本才从账号服务所在 Docker 网络向真实目标执行匿名 `PING`；目标要求认证时，从 Redis 容器环境键或独立 `--requirepass` 参数读取密码，再从同一网络使用 `REDISCLI_AUTH` 验证，并仅通过当前部署进程传递给 Compose。各分支只记录不含地址和密码的判定结果；密码不写入日志、GitHub、仓库或最终镜像。远程 Redis、ACL 文件和自定义配置文件保持显式配置模式。
