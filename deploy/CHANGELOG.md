...
- 关联版本：`@wlisfes/chat-web-base-schema@1.4.8`；Account 本次完整 Git SHA 镜像。
- 变更内容：升级共享包并将完整扁平化 `NacosRuntimeOptions` 从 `.env` 显式注入 `NacosModule.forRoot`；服务名和注册端口保留 Account 默认值，其余可选字段使用共享包默认值。环境示例补充请求超时及每项默认行为。
