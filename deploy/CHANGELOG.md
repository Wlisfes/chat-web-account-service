...
- 影响机器：`chat-home-server`。
- 事故级别：P0。强制 `NACOS_REGISTER_IP=10.66.0.2` 后，`10.66.0.2:5010` 超时，同机 Gateway `/api/account/health` 业务 503。
- 根因：同机 Gateway 不能把 WireGuard 地址当上游；Docker Desktop 对 WG 网卡端口映射不通。
- 正确处置：生产不设置 `NACOS_REGISTER_IP`，注册容器网卡 IP。只有远端 Gateway 已证实可达时才允许跨主机注册。
- 禁止项：不要把 `10.66.0.2` 写回 Account 生产 `.env`；不要改 CRM。
- 验证命令：Gateway 与公网 `GET /api/account/health` 返回业务 `status=UP`。
- 跨服务主记录：Gateway `deploy/RUNBOOK.md`。

## 2026-09-17：停止强制注册不可达的 Nacos 地址

- 影响机器：`chat-home-server`。
- 变更内容：部署不再默认写入 `NACOS_REGISTER_IP=10.66.0.2`。未显式配置 GitHub variable 时从 `.env` 删除该项，服务自动探测容器网卡 IP，供同机 Gateway 访问。若设置了 `NACOS_REGISTER_IP`，`deploy.sh` 会从 Gateway 探测该地址:5010。
- 机器侧操作：生产 `.env` 已删除 `NACOS_REGISTER_IP` 并重建容器。跨主机 `10.66.0.2:5010` 仍受 Docker Desktop 对 WireGuard 网卡端口映射限制。
- 验证命令：Gateway `GET /api/account/health` 返回业务 `status=UP`。
...
- 变更内容：部署时强制维护 `NACOS_REGISTER_IP`，默认使用 `10.66.0.2`；Account 容器发布 `5010` 到宿主机，使云端和本地 Gateway 都能访问 Nacos 注册实例。
- 机器侧操作：部署后确认 Nacos 中 Account 实例为 `10.66.0.2:5010`，并以管理员运行 Gateway 仓库的 WireGuard 防火墙脚本。
- 验证命令：检查 `docker ps` 端口、`Test-NetConnection 10.66.0.2 -Port 5010` 和 Gateway `/api/account/health`。
...
- 机器侧操作：更新部署目录 `.env` 的 `NACOS_SERVER`，重建 Account 容器并确认健康检查通过。
...
- 关联版本：Account 完整 Git SHA。
- 变更内容：移除 `chat-server-company` 部署任务，只保留 `chat-server-home` 和 `production-home`。
- 机器侧操作：Home Runner 保留 `chat-server-home` 标签；无需修改 `.env`、端口、Nacos 或部署目录。
- 验证命令：检查 Actions 的 `Deploy to home`、容器健康状态和 `/health`。
- 回滚方法：恢复上一完整 Git SHA；不得恢复已废弃的 Company 部署任务。

## 2026-09-07：彻底移除 Account 客户域残留

- 影响机器：Home（`chat-server-home`）。
- 关联版本：`@wlisfes/chat-web-base-schema@1.6.4` 与 Account 完整 Git SHA。
