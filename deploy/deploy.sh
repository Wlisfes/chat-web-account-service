...
while [ "$elapsed" -lt "$HEALTH_TIMEOUT" ]; do
    ...
    case "$state" in
        healthy)
            if ! docker exec "$CONTAINER" node -e "require('http').get('http://127.0.0.1:5010/health', response => process.exit(response.statusCode === 200 ? 0 : 1)).on('error', () => process.exit(1))"; then
                echo "Account health endpoint failed after the container became healthy." >&2
                rollback
                exit 1
            fi
            ...
            if [ -n "$register_ip" ]; then
                ...
                if ! docker exec -e CHECK_IP="$register_ip" chat-web-gateway-service node -e 'fetch("http://" + process.env.CHECK_IP + ":5010/health").then(async response => { const body = await response.text(); console.log("Nacos register IP probe status=" + response.status + " body=" + body); if (response.status !== 200) process.exit(1); }).catch(error => { console.error(String(error)); process.exit(1); })'; then
                    echo "NACOS_REGISTER_IP=${register_ip}:5010 is not reachable from Gateway. Docker port publishing to the WireGuard address may not work on Windows. Unset NACOS_REGISTER_IP so the service registers the container network IP." >&2
                    rollback
                    exit 1
                fi
            fi
